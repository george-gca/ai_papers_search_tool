...
class PaperFinderTrainer(PaperFinder):
    ...
    def _create_ngrams_count(self, word_list: list[str], n: int, top_n: int) -> list[tuple[list[str], int]]:
        ngrams = (
            w
            for w in tqdm(
                zip(*(word_list[i:] for i in range(n))),
                total=len(word_list) + 1 - n,
                unit="ngram",
                desc=f"Creating {n}-grams",
                ncols=TQDM_NCOLS,
            )
            if self.unk not in w and self.eop not in w
        )
    ...
    def _create_ngrams_set(self, word_list: list[str], n: int, ngram_threshold: int = 1000) -> set[str]:
        ...
        ngrams_counts = list(
            ("_".join(ngram), count) for ngram, count in takewhile(lambda x: x[1] >= ngram_threshold, ngrams_counts)
        )
    ...
    def build_dictionary(self, rebuild=False) -> None:
        ...
        self.words = list(
            w if w in self.dictionary else self.unk
        ...
        self.logger.print(
            f"Finished building dictionary with {len(self.dictionary):n} words.\n"
    ...
    def build_paper_vectors(
        ...
        # self.papers is built from paper_info.feather
    ...
    def build_similar_dictionary(self, count: int = 5) -> None:
        similar_dictionary = {
            word: self.get_most_similar_words(word, count)
            for word in tqdm(self.words, desc="Creating dictionary of similar words", unit="word", ncols=TQDM_NCOLS)
        }
    ...
    def convert_text_with_phrases(
        self, src_file: Path, dest_file: Path, column: str = "abstract", keep_na: bool = True
    ) -> None:
    ...
    def detect_ngrams(self, n: int, ngram_threshold: int = 1000) -> None:
        ...
        if len(ngrams_set) > 0:
            ...
            with tqdm(
                total=math.ceil(len(self.words) / chunk_size) * len(ngrams_set),
                unit="chunk",
    ...
    def reduce_paper_vectors_dim(self, new_dim: int, perplexity: int = 5, n_iter: int = 2000) -> None:
        ...
        with Timer(name="Reducing dimensions"):
            tsne = TSNE(
                ...
                init="pca",
                n_iter=n_iter,
                n_jobs=3 * multiprocessing.cpu_count() // 4,
            )
