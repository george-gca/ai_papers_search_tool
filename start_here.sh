...
if [ -n "$train_paper_finder" ]; then
    if [ -n "$create_for_app" ]; then
        ...
        $run_command python train_word_representation.py -l info -c $n_clusters -d 30000 -t $ngram_threshold --ngram_threshold_step $ngram_threshold_step -i --build_dictionary --detect_ngrams --train --min_year 2019
    fi
